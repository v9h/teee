...
if not import then return messagebox("Error 0x5; Something went wrong with initializing the script (couldn't load modules)", '', 0) end

-- Importing the modules and yielding until they are loaded
local ESP, Menu, Utils, Enums, Raycast, Network, Configs, Commands, ToolData, DoorData, Lighting, Settings, UserTable, TimerClass, PlayerManager
...
task.spawn(function() Settings = import("settings") end)
...
local Windows = {}
local Animations = {}
local AudioLogs = isfile(Settings.config_directory .. "Audios.dat") and string.split(readfile(Settings.config_directory .. "Audios.dat"), '\n') or {}
local BulletLogs = {}
...
do
	...
	UserTable.Whitelisted = isfile(Settings.config_directory .. "Whitelist.dat") and string.split(readfile(Settings.config_directory .. "Whitelist.dat"), '\n') or {}
	UserTable.Owners = isfile(Settings.config_directory .. "Owners.dat") and string.split(readfile(Settings.config_directory .. "Owners.dat"), '\n') or {}

	if not isfolder(Settings.name) then makefolder(Settings.name) end
	if not isfolder(Settings.name .. "/Games") then makefolder(Settings.name .. "/Games") end
	if not isfolder(Settings.name .. "/Games/The Streets") then makefolder(Settings.name .. "/Games/The Streets") end
	if not isfolder(Settings.config_directory .. "bin") then makefolder(Settings.config_directory .. "bin") end
	if not isfolder(Settings.config_directory .. "Luas") then makefolder(Settings.config_directory .. "Luas") end
	if not isfolder(Settings.config_directory .. "Configs") then makefolder(Settings.config_directory .. "Configs") end
	if not isfolder(Settings.config_directory .. "bin/sounds") then makefolder(Settings.config_directory .. "bin/sounds") end
	if not isfolder(Settings.config_directory .. "bin/skyboxes") then makefolder(Settings.config_directory .. "bin/skyboxes") end
	if not isfolder(Settings.config_directory .. "bin/crosshairs") then makefolder(Settings.config_directory .. "bin/crosshairs") end
	if not isfolder(Settings.config_directory .. "bin/replays") then makefolder(Settings.config_directory .. "bin/replays") end
	if not isfolder(Settings.config_directory .. "bin/models") then makefolder(Settings.config_directory .. "bin/models") end

	-- We read the values before this
	writefile(Settings.config_directory .. "Whitelist.dat", table.concat(UserTable.Whitelisted, '\n'))
	writefile(Settings.config_directory .. "Owners.dat", table.concat(UserTable.Owners, '\n'))

	if not isfile(Settings.config_directory .. "Spam.dat") then writefile(Settings.config_directory .. "Spam.dat", '') end
	if not isfile(Settings.config_directory .. "bin/sounds/hitsound.mp3") then writefile(Settings.config_directory .. "bin/sounds/hitsound.mp3", '') end
	if not isfile(Settings.config_directory .. "bin/crosshairs/crosshair.png") then writefile(Settings.config_directory .. "bin/crosshairs/crosshair.png", '') end
	if not isfile(Settings.config_directory .. "bin/crosshairs/reload_crosshair.png") then writefile(Settings.config_directory .. "bin/crosshairs/reload_crosshair.png", '') end

	HitSound = get_custom_asset(Settings.config_directory .. "bin/sounds/hitsound.mp3")
	Crosshair = get_custom_asset(Settings.config_directory .. "bin/crosshairs/crosshair.png")
end
...
function RenderStepped(Step: number)
	...
	if Config.Interface.Watermark.Enabled then
		Menu.Watermark:Update(Settings.name .. " | " .. GetFramerate() .. "fps | " .. math.round(Ping) .. "ms | " .. os.date("%X"))
	end
		...
		if (Method == "FireServer" or Method == "fireServer") and self.ClassName == "RemoteEvent" then
			...
			if not Caller and Arguments[1] == "play" and Arguments[2] then
				...
				if not table.find(AudioLogs, LastAudio) then
					...
					writefile(Settings.config_directory .. "Audios.dat", table.concat(AudioLogs, '\n'))
				end
			end
		end
...
function InitializeCommands()
	...
	Commands.Add("fly", {"flight"}, "- enables 'flight'", FlyToggle)
...
function InitializeMenu()
	...
	Menu.Screen.Name = Settings.name
	Menu:SetTitle(Settings.name .. Utils.GetRichTextColor(".cc", Config.Menu.Accent:ToHex()))
	...
	Menu.ComboBox("Visuals", "World", "Skybox", Config.Enviorment.Skybox.Value, Utils.GetFolders(Settings.config_directory .. "bin/skyboxes/").Names, function(String)
	...
	Menu.CheckBox("Misc", "Main", "Chat spam", Spamming, function(Bool)
		...
		if not isfile(Settings.config_directory .. "Spam.dat") then
			writefile(Settings.config_directory .. "Spam.dat", '')
		end
	end)
	...
	Menu.ColorPicker("Settings", "Menu", "Menu accent", Config.Menu.Accent, 0, function(Color)
		...
		Menu:SetTitle(Settings.name .. Utils.GetRichTextColor(".cc", Color:ToHex()))
	end)
	...
	Menu.Button("Settings", "Settings", "Refresh", function()
		...
		Menu:FindItem("Visuals", "World", "ComboBox", "Skybox"):SetValue(Config.Enviorment.Skybox.Value, Utils.GetFolders(Settings.config_directory .. "bin/skyboxes/").Names)
		Menu:FindItem("Settings", "Configs", "ListBox", "Configs"):SetValue(cfg_name .. ".cfg", Utils.GetFiles(Settings.config_directory .. "Configs").Names)

		HitSound = get_custom_asset(Settings.config_directory .. "bin/sounds/hitsound.mp3") -- huh seems to automatically when file changes?
		Crosshair = get_custom_asset(Settings.config_directory .. "bin/crosshairs/crosshair.png")
		Mouse.Icon = Crosshair
	end)

	Menu.TextBox("Settings", "Configs", "Config name", '')
	Menu.ListBox("Settings", "Configs", "Configs", false, Utils.GetFiles(Settings.config_directory .. "Configs/").Names, function(cfg_name)
	...
	Menu.Button("Settings", "Configs", "Delete", function()
		...
		Menu.Prompt("Are you sure you want to delete '" .. cfg_name .. "' ?", function()
			delfile(Settings.config_directory .. "Configs/" .. cfg_name)
			Menu:FindItem("Settings", "Configs", "ListBox", "Configs"):SetValue('', Utils.GetFiles(Settings.config_directory .. "Configs").Names)
		end)
	end)
...
function Initialize()
	if not HookGame() then
		return messagebox("Error 0x4; Failed to hook games anticheat!", Settings.name .. ".cc", 0)
	end
	...
	FloatPart.Name = Settings.name .. ".Float"
	...
	coroutine.wrap(function()
		while true do
			...
			pcall(function()
				local Messages = string.split(readfile(Settings.config_directory .. "Spam.dat"), '\n')
